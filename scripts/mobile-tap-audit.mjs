...
for (const path of PAGES) {
  ...
  const result = await page.evaluate(async () => {
    const els = [...document.querySelectorAll("a[href], button")].filter(el => {
      ...
      if (cs.pointerEvents === "none" || parseFloat(cs.opacity) === 0)
        return false;
  ...
  const realFails = result.fails.filter(
    f => !f.blocker.includes("manus-previewer-root")
  );
...
// 實測手機固定 CTA，並只在元件於手機版可見時測試桌面 WhatsApp 浮窗。
await page.goto(BASE + "/", { waitUntil: "networkidle" });
await page.waitForTimeout(400);
const mobileCtaTest = await page.evaluate(() => {
  const cta = document.querySelector('[data-mobile-cta="true"]');
  if (!cta) return { error: "找不到手機固定 CTA" };
  const style = getComputedStyle(cta);
  const links = [...cta.querySelectorAll("a[href]")];
  const visible =
    style.display !== "none" &&
    style.visibility !== "hidden" &&
    Number(style.opacity) !== 0 &&
    cta.getBoundingClientRect().height > 0;
  const clickable = links.every(el => {
    const r = el.getBoundingClientRect();
  ...
  return { visible, clickable, links: links.length };
});
console.log("Mobile CTA 實測:", JSON.stringify(mobileCtaTest));
if (mobileCtaTest.error || !mobileCtaTest.visible || !mobileCtaTest.clickable)
  totalFails++;

const widgetTest = await page.evaluate(async () => {
  const widget = document.querySelector('[data-whatsapp-widget="true"]');
  if (!widget) return { error: "找不到 WhatsApp widget" };
  if (getComputedStyle(widget).display === "none") {
    return { skipped: true, reason: "手機版按設計隱藏桌面 WhatsApp widget" };
  }
  const openBtn = document.querySelector(
    'button[aria-label="開啟 WhatsApp 對話框"]'
  );
  ...
  return {
    openOk,
    innerAllClickable: inner.every(Boolean),
...
if (
  ...
  (widgetTest.error ||
    !widgetTest.openOk ||
    !widgetTest.innerAllClickable ||
    widgetTest.closedPe !== "none" ||
    !widgetTest.closedNotBlocking)
)
  totalFails++;
