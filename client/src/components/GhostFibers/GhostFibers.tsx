...
void main() {
  ...
  vec3 backdrop = mix(uBackgroundColor, vec3(1.0), step(0.5, uLightMode));
...
export default function GhostFibers({
  ...
  useEffect(() => {
    ...
    let frameRate = Math.min(Math.max(fps, 1), 60);
    let isPaused = paused;
    let isVisible = true;
    ...
    contexts.set(container, {
      ...
      setFps(value) {
        frameRate = Math.min(Math.max(value, 1), 60);
      },
    });
  ...
  useEffect(() => {
    ...
    setColor(uniforms.uBackgroundColor, backgroundColor);
