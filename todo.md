# 本輪任務：地區著陸頁 + 工程案例 + 估價計算機（歷史紀錄）

> 2026-09-23 更新：互動式估價計算機及匿名估價記錄已移除。以下相關勾選項是過往交付證據，不代表現行網站仍提供該功能。
...
# 第十一輪任務：搜尋列即時篩選跳轉 + 地圖顏色對比 + tooltip 到達時間

- [x] 審視 Areas.tsx 現有頂部搜尋列實作，規劃即時篩選建議下拉（autocomplete）
- [x] 搜尋列輸入時即時顯示符合的地區建議清單（含所屬分區、專頁標記），點擊建議直接跳轉：有專頁 → /areas/{slug}；無專頁 → 捲動至對應分區籤並高亮
- [x] 鍵盤支援：上下鍵選擇、Enter 跳轉、Esc 關閉建議
- [x] 微調互動地圖三大分區填色以提升對比度（hki #16264f / kln #149e4c / nt #4a76ad，fillOpacity 0.78，白邊 1.4px）
- [x] hover tooltip 加入該區預計到達時間（45/60 分鐘內）
- [x] 截圖驗證（桌面 + 手機 375px）+ TypeScript/vitest 通過（12/12）
- [ ] 儲存檢查點並交付
